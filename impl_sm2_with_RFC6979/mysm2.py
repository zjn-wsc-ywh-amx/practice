...
class CryptSM2(object):
    ...
    def __init__(self, private_key, public_key, ecc_table=default_ecc_table):#初始化，规定各项数值
    ...
    def _double_point(self, Point):  #倍点运算
        l = len(Point)
    ...
    def encrypt(self, data):
        ...
        k1 = func.random_hex(self.para_len)
        k = rfc(data,int(priv,16))
        k += int(k1,16)
        # k=hex(k)
        # k1 = str(k)
        # p = self.para_len//len(k1) +1
        # print(self.para_len)
        # k1=(k1*p)[:self.para_len]
        # k=int(k1,16)
        # C1 = self._kg(int(k,16),self.ecc_table['g'])
        # xy = self._kg(int(k,16),self.public_key)
        C1 = self._kg(k,self.ecc_table['g'])
        xy = self._kg(k,self.public_key)
